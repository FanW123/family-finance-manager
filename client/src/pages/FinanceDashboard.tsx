...
const getAllTrackableCategories = (categories: any[], budgetType?: string): any[] => {
  ...
  categories.forEach(cat => {
    if (cat.isParent && cat.children) {
      ...
      // 去重：基于 id 去重子类（修复 double 计算 bug）
      const uniqueChildren = Array.from(
        new Map(childrenOfType.map((c: any) => [c.id, c])).values()
      );
      ...
      if (uniqueChildren.length > 0) {
        result.push({
          ...cat,
          isGroupSummary: true,
          trackableChildren: uniqueChildren
        });
      }
...
const FinanceDashboard = () => {
  ...
  // Auto-expand categories when opening add expense modal
  useEffect(() => {
    if (showAddExpense && budgetCategories) {
      const expanded = new Set<string>();
      budgetCategories.forEach((cat: any) => {
        if (cat.isParent && cat.children && cat.children.length > 0) {
          expanded.add(cat.id);
        }
      });
      setExpandedExpenseCategories(expanded);
    }
  }, [showAddExpense, budgetCategories]);
            ...
            {!showBudgetWizard && budgetCategories && (
              <div>
                ...
                {(() => {
                  ...
                  const trackableCategories = getAllTrackableCategories(budgetCategories, budgetTrackingTab === 'weekly' ? 'weekly' : budgetTrackingTab === 'monthly' ? 'monthly' : 'yearly');
                  ...
                  let startDate: Date;
                  let endDate: Date = new Date(budgetYear, budgetMonth, 0); // Last day of selected month
                ...
                {/* Original Summary Card - Hidden for now */}
                <div style={{ display: 'none' }}>
              ...
              <div style={{ padding: '2rem' }}>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <label style={{ fontSize: '0.9rem', color: COLORS.textMuted }}>
                      支出类别
                    </label>
                    <button
                      onClick={() => {
                        ...
                        <div key={cat.id}>
                          ...
                          <div
                            style={{
                              ...
                              padding: '0.75rem 1rem',
                              cursor: 'pointer',
                              background: newExpense.category === cat.id ? `${COLORS.highlight}30` : 'transparent',
                          ...
                          >
                            ...
                            {cat.isParent && cat.children && cat.children.length > 0 && (
                              <span
                                ...
                                style={{
                                  ...
                                  color: COLORS.textMuted,
                                  transition: 'transform 0.2s',
                                  transform: expandedExpenseCategories.has(cat.id) ? 'rotate(90deg)' : 'rotate(0deg)',
                            ...
                            <div
                              ...
                              style={{
                                ...
                                color: COLORS.text,
                                fontSize: '0.95rem',
                                paddingLeft: (!cat.isParent || !cat.children || cat.children.length === 0) ? '1.3rem' : '0'
                              }}
                            >
                              {newExpense.category === cat.id && <span style={{ marginRight: '0.5rem' }}>✓</span>}
                              {cat.name}
                            </div>

                            {/* Quick Add Button for Parent Categories */}
                            {cat.isParent && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  // Quick add expense to this category
                          ...
                          {/* Child Categories */}
                          {cat.isParent && cat.children && expandedExpenseCategories.has(cat.id) && (
                            <div style={{ background: COLORS.primary }}>
                              {cat.children.map((child: any) => (
                                <div
                                  key={child.id}
                                  onClick={() => setNewExpense({ ...newExpense, category: child.id })}
                                  style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    padding: '0.6rem 1rem 0.6rem 3rem',
                                    cursor: 'pointer',
                                    background: newExpense.category === child.id ? `${COLORS.highlight}30` : 'transparent',
                                    borderBottom: `1px solid ${COLORS.accent}`,
                                    transition: 'background 0.2s'
                                  }}
                                  onMouseEnter={(e) => {
                                ...
                                >
                                  <span style={{ marginRight: '0.5rem', color: COLORS.textMuted, fontSize: '0.8rem' }}>↳</span>
                                  <span style={{ color: COLORS.text, fontSize: '0.9rem', flex: 1 }}>
                                    {newExpense.category === child.id && <span style={{ marginRight: '0.5rem' }}>✓</span>}
                                    {child.name}
                                  </span>
                                </div>
                              ))}
                        ...
                        <div key={cat.id}>
                          ...
                          <div
                            style={{
                              ...
                              padding: '0.75rem 1rem',
                              cursor: 'pointer',
                              background: editingExpense.category === cat.id ? `${COLORS.highlight}30` : 'transparent',
                              borderBottom: `1px solid ${COLORS.primary}`,
                              transition: 'background 0.2s'
                            }}
                            onMouseEnter={(e) => {
                          ...
                          >
                            ...
                            >
                              {editingExpense.category === cat.id && <span style={{ marginRight: '0.5rem' }}>✓</span>}
                              {cat.name}
                            </div>
                          ...
                          {cat.isParent && cat.children && expandedExpenseCategories.has(cat.id) && (
                            <div style={{ background: COLORS.primary }}>
                              {cat.children.map((child: any) => (
                                <div
                                  key={child.id}
                                  onClick={() => setEditingExpense({ ...editingExpense, category: child.id })}
                                  style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    padding: '0.6rem 1rem 0.6rem 3rem',
                                    cursor: 'pointer',
                                    background: editingExpense.category === child.id ? `${COLORS.highlight}30` : 'transparent',
                                    borderBottom: `1px solid ${COLORS.accent}`,
                                    transition: 'background 0.2s'
                                  }}
                                  onMouseEnter={(e) => {
                                ...
                                >
                                  <span style={{ marginRight: '0.5rem', color: COLORS.textMuted, fontSize: '0.8rem' }}>↳</span>
                                  <span style={{ color: COLORS.text, fontSize: '0.9rem', flex: 1 }}>
                                    {editingExpense.category === child.id && <span style={{ marginRight: '0.5rem' }}>✓</span>}
                                    {child.name}
                                  </span>
                                </div>
                              ))}
                ...
                {totalPortfolio > 0 && (
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ color: COLORS.textMuted, marginBottom: '0.25rem' }}>💡 现值计算</div>
                      <div style={{ color: COLORS.text, fontWeight: '600' }}>
                        20年后的 $10,000,000 相当于现在的 ${(() => {
                          const inflationRate = rebalanceInflation / 100;
                          const presentValue = 10000000 / Math.pow(1 + inflationRate, 20);
                          return Math.round(presentValue).toLocaleString();
                        })()}
                      </div>
                      <div style={{ fontSize: '0.75rem', color: COLORS.textMuted, marginTop: '0.25rem' }}>
                        (基于 {rebalanceInflation > 0 ? rebalanceInflation.toFixed(1) : '0'}% 通胀率)
                      </div>
                    </div>
                    <div style={{
                      padding: '0.75rem',
                ...
                <div style={{
                  ...
                  {(() => {
                    ...
                    for (let year = 1; year <= rebalanceYears; year++) {
                      ...
                      // Withdrawal at the beginning of year (adjusted for inflation)
                      ...
                      let remainingWithdrawal = inflationAdjustedWithdrawal;
                      let afterWithdrawalStocks = yearStartStocks;
                      let afterWithdrawalBonds = yearStartBonds;
                      let afterWithdrawalCash = yearStartCash;
                      
                      // Step 1: Withdraw from cash
                      if (remainingWithdrawal > 0 && afterWithdrawalCash > 0) {
                        const cashWithdrawal = Math.min(remainingWithdrawal, afterWithdrawalCash);
                        afterWithdrawalCash -= cashWithdrawal;
                        remainingWithdrawal -= cashWithdrawal;
                      }
                      
                      // Step 2: If cash is not enough, sell bonds (priority)
                      if (remainingWithdrawal > 0 && afterWithdrawalBonds > 0) {
                        const bondSale = Math.min(remainingWithdrawal, afterWithdrawalBonds);
                        afterWithdrawalBonds -= bondSale;
                        remainingWithdrawal -= bondSale;
                      }
                      
                      // Step 3: If bonds are not enough, sell stocks
                      if (remainingWithdrawal > 0 && afterWithdrawalStocks > 0) {
                        const stockSale = Math.min(remainingWithdrawal, afterWithdrawalStocks);
                        afterWithdrawalStocks -= stockSale;
                        remainingWithdrawal -= stockSale;
                      }
                      
                      // If still not enough, assets are depleted
                      if (remainingWithdrawal > 0) {
                        // Assets depleted, stop simulation
                        simulationData.push({
                          year,
                          yearStartTotal,
                          yearStartStocks,
                          ...
                          <table style={{
                            ...
                            <tbody>
                              {simulationData.map((row, index) => {
                                const rebalancedTotal = row.rebalancedStocks + row.rebalancedBonds + row.rebalancedCash;
